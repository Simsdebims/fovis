...
MotionEstimator::MotionEstimator(const CameraIntrinsicsParameters params, const VisualOdometryOptions& options) : _params(params)
{
  _ref_frame = NULL;
  _target_frame = NULL;
  ...
  VisualOdometry::getDefaultOptions(&defaults);
...
void MotionEstimator::matchFeatures(PyramidLevel* ref_level, PyramidLevel* target_level)
{
  ...
  Eigen::Matrix<double, 3, 4> xyz_c_to_uvw_c = _params.toProjectionMatrix();
  ...
  for (int ref_ind = 0; ref_ind < num_ref_features; ref_ind++) {
    ...
    Eigen::Vector2d ref_uv(ref_level->getKeypointBaseU(ref_ind),
                           ref_level->getKeypointBaseV(ref_ind));
    std::vector<int>& ref_candidates(candidates[ref_ind]);
    for (int target_ind = 0; target_ind < num_target_features; target_ind++) {
      Eigen::Vector2d target_uv(target_level->getKeypointBaseU(target_ind),
                                target_level->getKeypointBaseV(target_ind));
...
void MotionEstimator::computeMaximallyConsistentClique()
{
  ...
  double stereo_focal_length = _params.fx;
#endif
  ...
  int* reject = new int[_num_matches];
...
void MotionEstimator::refineMotionEstimate()
{
  ...
  for (int m_ind = 0; m_ind < _num_matches; m_ind++) {
    ...
    target_projections.col(i) = match.refined_target_keypoint.base_uv;

    ref_xyz.col(i) = match.ref_keypoint->xyzw;
    ref_projections.col(i) = match.ref_keypoint->base_uv;
    i++;
  }
...
void MotionEstimator::computeReprojectionError()
{
  ...
  Eigen::Matrix<double, 3, 4> proj_matrix = _params.toProjectionMatrix();
  ...
  for (int m_ind = 0; m_ind < _num_matches; m_ind++) {
    ...
    Eigen::Vector2d err = match.ref_keypoint->base_uv - reproj;
