...
{
	float val = _depth_data[rgbToDepthIndex(u, v)];
...
DepthImage::getXyz(OdometryFrame * frame) 
{
  ...
  for(int level_num=0; level_num < num_levels; ++level_num) {
    ...
    for(int kp_ind=0; kp_ind < num_kp; ++kp_ind) {
      ...
      int u = (int)(kpdata->base_uv(0)+0.5);
      int v = (int)(kpdata->base_uv(1)+0.5);
...
{
  float u_f = kpdata->base_uv(0);
  float v_f = kpdata->base_uv(1);
