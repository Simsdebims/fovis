 ...
 * TODO
 */
class FOVIS_EXPORT PyramidLevel
{
  public:
    ...
    float getKeypointBaseU(int kp_index) const {
      return _keypoints[kp_index].base_uv(0);
    }

    float getKeypointBaseV(int kp_index) const {
      return _keypoints[kp_index].base_uv(1);
    }

    const Eigen::Vector2d& getKeypointBaseUV(int kp_index) const {
      return _keypoints[kp_index].base_uv;
    }
