...
OdometryFrame::OdometryFrame(const CameraIntrinsicsParameters& params, const VisualOdometryOptions& options)
{
  const CameraIntrinsicsParameters& input_camera = params;
                            ...
                            DepthSource* depth_source)
{
  ...
  for (int level_num=0; level_num<_num_levels; level_num++) {
    ...
    for (int kp_ind=0; kp_ind<num_kp_candidates; kp_ind++) {
      ...
      int du = static_cast<int>(kpdata.base_uv(0)+0.5);
      int dv = static_cast<int>(kpdata.base_uv(1)+0.5);
